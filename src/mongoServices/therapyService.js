...
const findAllQuery = async (query) => {
	...
	const data = await therapyModel
		.find(whereClause)
		.skip(page > 0 ? +limit * (+page - 1) : 0)
		.limit(+limit || 20)
		.sort(sort);
	const totalCount = await therapyModel.find(whereClause).countDocuments();
...
const updateOneQuery = async (filter, update, projection) => {
	...
	const data = await therapyModel.findOneAndUpdate(filter, update, options);
	return data;
};
